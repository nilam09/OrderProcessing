...
async function fetchOrders() {
  ...
  try {
    ...
    const orderList = Array.isArray(orders)
      ? orders
      : orders && Array.isArray(orders.content)
        ? orders.content
        : [];
    if (!orderList.length) {
      tbody.innerHTML =
...
async function loadOrderDetails(id) {
  ...
  try {
    const res = await fetch(`${API_BASE}/${id}?expand=products`, {
      headers: getAuthHeaders(),
    });
    if (!res.ok) {
      container.innerHTML =
        '<p class="text-danger mb-0">Failed to load order details.</p>';
      return;
    }
    ...
    try {
      ...
      if (userRes.ok) {
        ...
        document.getElementById("order-details").innerHTML =
          '<p class="text-muted mb-0">Select an order to see details.</p>';
      } else {
